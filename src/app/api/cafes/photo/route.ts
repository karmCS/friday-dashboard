/**
 * `POST /api/cafes/photo` — attach a photo to an existing cafe.
 *
 * Accepts a `multipart/form-data` body with:
 *   - `cafe_id`  (text)  — the cafe to attach the photo to
 ...
 * generated, collision-resistant name, and its path is stored on the cafe row's
 * `photo_path` column. Returns the updated cafe.
 *
...
import { error, json, parseId, toPublicCafe, type CafeRow } from "../shared";
...
const SELECT_SQL = `SELECT id FROM cafes WHERE id = ?`;

const UPDATE_PHOTO_SQL = `
  UPDATE cafes SET photo_path = @photo_path WHERE id = @id
...
export async function POST(request: Request): Promise<Response> {
  ...
  const cafeIdRaw = form.get("cafe_id");
  if (typeof cafeIdRaw !== "string") {
    return error("cafe_id is required", 400);
  }
  const cafeId = parseId(cafeIdRaw);
  if (cafeId === null) return error("invalid cafe_id", 400);
  ...
  // Confirm the cafe exists before reading/writing anything.
  let exists: { id: number } | undefined;
  try {
    exists = getDb().prepare(SELECT_SQL).get(cafeId) as { id: number } | undefined;
  } catch {
    return error("failed to look up cafe", 500);
  }
  if (!exists) return error("cafe not found", 404);
  ...
  // The `cafe-` prefix keeps cafe and taco photos visually distinct in the shared store.
  const filename = `cafe-${cafeId}-${randomUUID()}${ext}`;
  ...
  try {
    const updated = getDb()
      ...
      .get({ id: cafeId, photo_path: absPath }) as CafeRow | undefined;
