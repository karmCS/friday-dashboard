/**
 * `GET /api/cafes/[id]/photo` — stream the image attached to a cafe.
 *
 * The cafe's `photo_path` is an absolute server-filesystem path (written by
 * `POST /api/cafes/photo`), which the browser cannot load directly — this route reads it back.
 * Lookups are by cafe id only; the stored path is never taken from the client, and we still
...
/** Photo storage dir on the mounted volume (matches api/cafes/photo/route.ts). */
...
const SELECT_SQL = `SELECT photo_path FROM cafes WHERE id = ?`;
...
export async function GET(_request: Request, context: RouteContext): Promise<Response> {
  ...
  if (!row.photo_path) return error("no photo for this cafe", 404);
  ...
  try {
    ...
    return new Response(new Uint8Array(bytes), {
      ...
      headers: {
        ...
        // Stable URL per cafe, so don't cache hard — a re-upload must show immediately.
