/**
 * Shared types, validation, and helpers for the Cafe Tracker API routes.
 *
 * Data model (analytics-dashboard.md → Cafe Tracker, a sibling of the Taco Tracker):
 *   cafes(id, place, city, state, order_item, rating, price_tier, notes,
 *         photo_path, visited_at, created_at)
 *
 * `order_item` is the cafe analogue of the taco's `taco_type` (the drink/item ordered).
 * The `cafes` table is created idempotently by `getDb()` (see @/lib/db). These routes own
 * CRUD + photo upload. All input is validated at the boundary before it touches SQL, and
 * every query is parameterized (never string-built).
 */

/** A cafe row as stored in SQLite. */
...
/**
 * A cafe as exposed to the client. Identical to {@link CafeRow} except the on-disk
 ...
 */
export type PublicCafe = Omit<CafeRow, "photo_path"> & { has_photo: boolean };

/** Project a stored row into its client-safe shape (drops the absolute photo path). */
export function toPublicCafe(row: CafeRow): PublicCafe {
...
/** Validated fields for creating a cafe (POST). Required fields are non-optional here. */
...
/** Validated, partial fields for updating a cafe (PATCH). Only present keys are applied. */
export type CafePatchInput = Partial<CafeCreateInput>;
...
/** Validates a POST body into a {@link CafeCreateInput}. */
export function validateCreate(body: unknown): ValidationResult<CafeCreateInput> {
  if (!isRecord(body)) {
  ...
  const order_item = requireString(body.order_item, "order_item", errors);
...
/**
 * Validates a PATCH body into a {@link CafePatchInput}. Only keys actually present in the
 * body are validated and returned — absent keys are left untouched on update. Requires at
 * least one updatable field.
 */
export function validatePatch(body: unknown): ValidationResult<CafePatchInput> {
  if (!isRecord(body)) {
    return { ok: false, errors: ["request body must be a JSON object"] };
  }
  const errors: string[] = [];
  const patch: CafePatchInput = {};
  ...
  if ("order_item" in body) {
    const v = requireString(body.order_item, "order_item", errors);
    if (v !== undefined) patch.order_item = v;
  }
  if ("rating" in body) {
